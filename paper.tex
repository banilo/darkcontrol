...
In the thermodynamic limit ($n \rightarrow \infty$), statistical physics \textcolor{blue}{[Liu et al. 2011]} gives the extremely good estimate
\begin{equation}
...
As explained in \textcolor{blue}{[Liu et al. 2011]}, the choice of linear dynamics over a more general nonlinear dynamics can be justified as follows:
\begin{itemize}
...
We propose to use a linear quadratic regulator (LQR) for the feedback controller. Thus consider the time-varying \text{value function} $V: [0, \tau] \times \mathbb R^n \rightarrow \mathbb R$, defined by
\begin{equation}
  \begin{split}
  ...
  \label{eq:hj}
\end{equation}
where the matrices $\Q$ and $\R$, specified with design considerations, are restricted to be positive semi-definite and positive definite, respectively. A classical calculation (remnicent of the \textit{Pontryagin minimization principle}) reveals that the optimal feedback control in \eqref{eq:hj} is given by
\begin{equation}
  \u(t) = -\K(t)\x(t),
\end{equation}
where
\begin{equation}
  \K(t) := \R^{-1}\B^T\P(t)
\end{equation}
and the time-varying matrix $\P(t)$ solves following matricial Riccati differential equation
\begin{equation}
\begin{split}
