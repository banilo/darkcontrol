...
Given a utility function $U$ on a probability space $(\Omega, \mathcal F, *)$, the \textit{Gibbs measure} at temperature $\alpha > 0$ and energy levels $(-U(\omega))_{\omega \in \Omega}$ is defined to be the probability measure (on thesame measurable space)
  ...
  \begin{equation}
    J(P'|U,P) \le U(\Omega) = -\alpha \log(Z_{U}(\alpha)).
    \end{equation}
  ...
  \begin{equation}
    \Delta J_{(U_{\text{ini}}, P_{\text{ini}}) \rightarrow P_{\text{fin}}} := J_{\text{fin}} - J_{\text{ini}} = \underbrace{\mathbb E_{P_{\text{fin}}}(\Delta U)}_{\textbf{accuracy}} + \alpha \underbrace{D_{\text{KL}}(P_{\text{ini}}\|P_{\text{fin}})}_{\textbf{complexity}},
    \label{eq:free}
  \end{equation}
