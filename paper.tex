...
In this section we will develop from first-principles, the bare-bones minimalistic ideas needed to build a free-energy principle for general decision-making. This ideas were first developed by Hinton et al. in the early 90s in the form of the so-called  \textit{Helmholtz machine}, a hierarchical factorial directional deep-belief net. Theories like Friston's free-energy principle and active-inference will then emerge as particular instances of this general framework, with particular design choises. For example the Friston theory axiomatizes that the brain uses a (problematic, as it implicitly assumes that posterior of each hidden unit is factorial, e.g) wake-sleep algorithm to train the underlying Helmholtz machine, etc.
\begin{table}[H]
...
\begin{eqnarray}
  ...
  \label{eq:helm}
\end{eqnarray}
where $E_G(\psi, \zeta)$ is the energy of a fictive thermodynamic system defined by setting
\begin{equation}
...
...
\subsection{Friston's active-inference}
...
\subsubsection{Free-energy minimization via a stochastic gradient variation bayes estimator}
\textbf{XXX see Kingma 2014 -- 2014: the poor boy is minimizing FE without knowing!}
\begin{equation}
  \begin{split}
    F_G^R(\zeta) &:= \langle E_G(., \zeta) \rangle_{p_R(. |\zeta)} - \mathcal H(p_R(. |\zeta)) = \langle E_G(., \zeta) - \log(p_R(.|\zeta))\rangle_{P_R(.|\zeta)}\\
    &=\langle \Delta E_{G \rightarrow R}(., \zeta)\rangle_{P_R(.|\zeta)}
    \end{split}
\end{equation}
