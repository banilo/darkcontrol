...
\begin{mdframed}
  \textbf{Time-invariance assumption:} It is assumed that the MDP has reached steady-state conditions so that
  the dynamics of the process are equivalent over sufficiently long time windows of equal length.
\end{mdframed}
...
a ``radar'' function to
detect subjectively salient object and unexpected events in the environment.
The sentinel account does however not provide a formal mechanisms of
...
This mechanism, mediated by the Bellman equation,
directly implies allocation of attention and computational load
to relevant objects and events in the environment.
%
Further,
our computational account provides an explanation for the known implication
of the DMN
in both certain goal-directed experimental tasks and
especially in the idlying rest cognitive set.
...
%
Consequently, the DMN has been proposed to reflect
...
Instead,
our formal account can parsimoneously explain the dominance of social content in
human mental scenes by their relevance to humans
reflected in high estimates for social information obtained from the value function
