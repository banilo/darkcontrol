...
major components of an efficient \textit{reinforcement learning (RL)} algorithm, and all these
...
\textbf{these first sentences are a little too abstract.
Take inspiration from here: An MDP represents an agent’s model of its interaction with its environment. MDPs encode all relevant information about the configuration of the world and the agent with the state variable. MDPs also represent the affordances of the environment: what actions the agent can take and a causal model of how these actions change the state of the world. Finally, MDPs represent the subjective rewards or costs caused by the agent’s actions in each state.}
...
\textbf{XXX say quite early what this control-theoretical object is optimizing
to inferring solutions to decision problems of what to do next or what action to take;
sequential decision making}
Instead, it must maintain a probability distribution over the set of possible states, based on a set of observations and observation probabilities, and the underlying Markov process. This is a minimal amount of assumptions that can be made about an environment, and is charastistic of so-called \textit{model-free RL}, the approach adapted by this work.
...
% It may be in the same order, or in reverse.
