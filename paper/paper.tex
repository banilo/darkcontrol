...
Here, $\langle . \rangle$ is the \textit{expectation} operator, $\mathcal H$ is the \textit{entropy} functional while $\operatorname{KL}(.|.)$ is the usual \textit{Kullback-Leibler (KL) divergence},
which is a measure of how different two probability distributions are. In this framework, the goal of the agent is then to iterative refine the generative model $p_G$ and the recognition model $p_R$ so as to minimize the free energy $F_G^R(\x)$ over sensory input $\x$.
