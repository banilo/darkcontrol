...
sampling probabilities over candidate actions to perform from that state. The  goal of RL is then
...
\begin{eqnarray*}
  \begin{split}
    ...
    \ldots\\
    a_t &\sim \pi(.|s_t)\\
    s_{t+1} &\sim \rho(.|s_t,a_t) \text{ and collect rewards }r(s_t, a_t)\\
    \ldots
  \end{split}
\end{eqnarray*}
