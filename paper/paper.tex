...
dedicated to minimizing the long-term average of surprise: the log-likelihood of the observed sensory input, or more precisely, an upper bound thereof. It does this by actively generating hypothetical explanations or predictions of its sensory input.
...
\subsubsection{Criticisms of the FEP}
  Despite it's popularity, a number of critics about the FEP have surfaced over the years. We outline the main ones here.
...
Since the FEP postulates that all ongoing intelligent behavior in an organism is geared towards reaching the grand goal of minimizing surprisal, an organism which enters a \textit{dark room}\footnote{\textit{Dark} is used here as an allegory to indicate the complete absence of any \textit{external} sensory input.} would strive to remain there as its sensory input would be perfectly predictable in such a state \citep{darkroom2012}.
\begin{wrapfigure}{r}{.45\linewidth}
  ...
  \label{fig:darkroom}
\end{wrapfigure}
However, this is definitely not what is observed, neither in humans nor in animals: if you get into a dark room, you try to figure out where the light switch is, so as to \textit{explore} this mysterious room, or simply get out of it! Defenders of the FEP have retaliated by advancing the `full package''\citep{darkroom2012}: FEP is multi-scale, and there would be a meta-scale at which the organism would
be \textit{surprised} by such a \textit{lack of surprise}. According to this argument, a dark room is thus paradoxically, a state of extremely high surprise. Driven by the zeal to always diminish surprise, the FEP agent would eventually bootstrap itself out of such saddles, to explore more interesting corners of the environment. Recourse to this argument is essential equivalent to invoking a \textit{halting oracle}, which is problematic as it sucks up the problem the theory is trying to address in the first place.

In contrast, operating under our RL-based theory, an organism would quickly notice that this dark room is a local minimum: hippocampal experience replay (see \ref{sec:self}) would set it and the organism would sample memories or fantasies of alternative situations in which there was reward to mine. Such considerations would artificially generate \textit{internal} sensory input and eventually let the organism to explore the room (for example look for and find the light switch), or simply leave the room. 
