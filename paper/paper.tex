...
transfer in the absence of concrete behavioral goal (Bzdok and Eickhoff, 2016a).
...
in the so-called "default mode network" (DMN) was discovered entirely by accident
(Shulman et al., 1997). This regrouping of some of the highest regions in
...
to allow for the discovery of a unique brain network that subserves a
set of baseline mental activity (Bzdok et al., 2012; Bzdok et al., 2015).



The "default-mode network" (DMN) appeared to be exclusive in task-induced
...
DMN activity also persists to a substantial degree during the early stages of
...
%
The overarching function of the DMN is mystified by
controlling the interplay between perception–action cycles and
maintaining a baseline contemplations
across time, splace, and content domains at the interface of external world
and self (Bzdok et al., 2015).



Consider an organism faced by choosing the next action
and being informed by rich
reenactment of really happened, hypothetically conceived, and
predicted future events to perfect ongoing decision making.
Control theory is a particularly attractive computational framework
to describe, quantify, and predict the autonomously acting systems,
perhaps including the human brain.
It is known that, the more external world is predictable,
...
(Mason et al., 2007)
that may contribute to optimized control of the organism.
A policy matrix captures the possible action than can be performed
on the environment given a current state. A value matrix 
...
In a model-free reinforcement learning approach,
we try to understand the environment always better
starting, at birth, with an environment that initially unknown.
Informed by gradient updates,
the adaptive DMN dynamics learned by feed-back loops
that are calibrated by error occurrence.
Reinforcement learning frameworks can naturally embed DMN function
into the tension between exploitative action with immediate reward and
explorative action with longer-term reward schedules.
The diversity of DMN implications in human cognitive processes
can be parsimoneously recast as minimizing prediction error
and maximizing reward outcome at various time scales.
Such a biologically meaningful optimization objective
...
may be purposeful as building blocks to iteratively improve
the predictive DMN machinery to optimize behavior of the organism.
...
into the mechanistic relevance of each individual network node
by reviewing experimental neuroscience evidence.
%
...
are now to adapt to novel situations and stimuli with difficulty
