...
reward outcomes into short- and longer-term behavioral agendas.
...
such as in \textbf{XXX COMPLETE-ME}
...
Isolated building blocks of world structure probably get reassembled
...
\begin{wrapfigure}{r}{.4\linewidth}
  \caption{\textbf{Reinforcement learning in a nutshell.} Given the current state of the environment,
    ...
    \textbf{XXX Please increase size of the font in the figure}}
  \centering
...
The goal of the RL agent is then to update this policy in order
to maximize $G^\pi$, in an average sense to be made precise latter. In
the definition of cumulative reward $G^\pi$ \eqref{eq:cumr},
the constant $\gamma$ $(0 \le \gamma < 1$) is the reward \textit{discount factor}, viewed to be specific an agent.
Setting $\gamma = 0$ corresponds to a perfectly shortsighted agent who
is hedonistic in being exclusively concerned with immediate rewards.
...
it is necessary to set $0 < \gamma < 1$.
...
Given only the limited information available from a MDP, at a state $s$ the average quality or usefulness of taking an action $a$ under a policy $\pi$ is completely captured by the single number
...
represents by no means, a candidate schema for solving MDPs with even moderately-sized action and state spaces: it's intractable!
...
HC --, possibly discarding the oldest entries to make space:
...
In the human brain, the HC is likely to contribute to such a mechanism,
...
\citep{silver2010monte}. Such methods have a sample complexity that is determined only by the underlying difficulty of the MDP, rather than the size of the state space or observation space,
...
and expected upcoming events to guide action choice in the present context.
