...
continuous evaluation and prediction of the environment to guide action choices.
%
...
may exert higher-order control on human behavior.
It follows the notion of "a hierarchy of brain systems with
...
simultaneously influence perception-action cycles and
...
to calibrate action trajectories to
...
etc., just to name a few. Such a learning scheme can be
plausibly realized in the human brain. Indeed,
...
synaptic plasticity at targets such as the ventral striatum.
...
The tensor $\theta$ encodes the model
  parameters and would correspond to synaptic weights and connection strengths between
  ...
  The oracle Q-values $Q(s, a|\theta)$ tell the DMN the (expected) usefulness of taking an action a in state / situation  $s$.
  The DMN, and in particular its dmPFC node, would then contribute to the choice, at a given $s$, of an action $a$ which maximizes this tabulated values. This mapping from states to actions defines a matrix, which can be conveniently called a "policy matrix". Learning consists in starting from a given instantiation of such a table and tweaking it around while using it to take actions which take the agent to different areas of the table, and so forth.


  \subsubsection{Self-training and the loss function}
Successful learning is challenging without a clear learning goal --
...
$Q$-value functions provably\footnote{The proof is standard and relies on the Banach Fixed-Point Theorem applied to the Bellman transform.}
...
for constant refinement of the organism's fate.
