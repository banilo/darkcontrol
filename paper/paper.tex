...
\textbf{\\keywords}: mind-wandering, artificial intelligence, Markov decision process
...
\begin{wrapfigure}{r}{.4\linewidth}
  ...
  % \url{www.github.com/banilo/darkcontrol_paper}.
  }
  \label{fig:dmn}
\end{wrapfigure}
...
Precisely, rewards that are not expected to come within
...
% INTRO
According to the free-energy principle (FEP) and
active inference theories \citep{friston2010free, fristonAIorRL},
the brain corresponds to
a biomechanical inference engine. It is
dedicated to minimizing the long-term average of surprise: the log-likelihood of the observed sensory input --or more precisely, an upper bound thereof-- \textit{relative} to the expectations about the external world derived from the brain's internal representations. The brain
would continuously generate hypothetical explanations of the world
and predict its sensory input.
Precursors of this theory can be traced back to Dayan and colleagues \citep{dayan1995helmholtz}
...
The FEP also postulates a companion model called the \textit{recognition} model,
...
while the generative model tries to produce sensations $\x$ which match these
...
\citep{dayan1995helmholtz}. As previously noted \citep{dayan1995helmholtz},
a crucial drawback of the wake-sleep algorithm is that it postulates a pair
...
Thus, the brain may therefore be unlikely to implement a variant of the wake-sleep algorithm.
We note that the recent theory of
\textit{variational auto-encoders} (VAEs) \citep{kingma2013auto} might provide an efficient alternative to the wake-sleep algorithm. This is because VAEs
...
Since FEP-based theories \citep{friston2010free,fristonAIorRL} conceptualize ongoing behavior
...
in this environmental state \citep{darkroom2012}.
...
\begin{wrapfigure}{r}{.45\linewidth}
  ...
  in a closed space is used as a thought experiment for the complete absence
...
Driven by surprise-minimization objective, the FEP agent would eventually bootstrap itself out of such saddles to explore more interesting parts of the environment.
...
In contrast, an organism operating under our RL-based theory would readily
identify the visual-input-free room as a local minimum. Indeed, hippocampal experience replay (see \ref{sec:self}) would serve to sample memories or fantasies of alternative situations
...
Finally, we note that FEP and active inference can be recast in our model-free RL framework.
This becomes possible by postulating that the reward signal maximized by the DMN is
the negative surprise,
that is, the log-likehood of sensory the current priors the agent has about the world. More explicitly, this corresponds to
...
Such a surprisal-based reinforcement learning scheme has previously been advocated \citep{schmidhuber2010,mohamed2015} under the equivalent framework of information compression.
Nevertheless, minimization of surprise alone
may be insufficient to explain behavior observed in humans and other intelligent animals.
