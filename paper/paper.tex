...
continuous.
%% The environment $\mathcal E$ is generally taken as stochastic, but having some kind of
%% memory.
...
Note that most RL algorithms, including \textit{Temporal Difference (TD)} learning [Sutton et al.,
  Bertsekas] and SARSA can be cast in this general variational framework. For example TD can corresponds
to the above framework with a linear value approximator with feature encoding $\phi(s,a) = \delta_{(s,a)} =  \text{ point mass at }(s,a)$
on the grid $\mathcal S \times \mathcal A$,
