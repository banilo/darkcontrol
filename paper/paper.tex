...
at intermediate levels, above thalamic and primary sensory
...
This computational account of DMN will be described in the mathematical framework of Markov Decision Processes (MDPs). MDPs indeed address specifically the case of decision making in stochastic context with reward feedback.
...
In contrast, model-free methods require no prespecified knowledge of the environment (transition probabilities, identities of sensory input, etc.) or representation thereof, and the agent learns which state-action pairs lead to reward by sampling the world in a trial-and-error manner, and inferring long-run reward aggregates, using prediction error as an incentive. In so doing, such an intelligent agent ultimately
learns both a policy how to act on the environment and an implicit representations of the world 
\citep{sutton1998reinforcement}. Model-free RL may struggle with high numbers of states, but scales better with the time horizon than model-based RL \citep{strehl2006}. Drawing from a vast literature, we hope to show that the DMN can be expected to harbor the neurobiological hardware necessary to implement the principles of a model-free RL agent.
...
This random variable\footnote{Random as it depends both on the environment's dynamic and the
  policy $\pi$ being played (which can be stochastic). The exponential delay discounting function used here refere to the usual formulation in the field of reinforcement learning but psychological experiments may also reveal other discounting regimes \citep{green2004discounting}}  measures the cumulative reward of
