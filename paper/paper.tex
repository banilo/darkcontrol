...
\begin{abstract}
The default mode network (DMN) reflects the  baseline mental activity
in humans.
%
...
The DMN is believed to be dedicated to
continuous autobiographical memory retrieval,
generation of hypothetical outcomes,
and reward contingency evaluation when letting the mind go.
Indeed, the DMN probably accounts for the
highest energy consumption in the brain and
is intimately coupled with conscious awareness.
%
...
\citep{shul1997}. This networked collection of some of brain regions at the top of
...
In the beginning of the 21st century, brain imaging
was perhaps the first existing technology
to allow for the discovery of a unique brain network that subserves
baseline mental activities
\citep{raichle2007default, bzdok2015resting}.
...
This processing of yet-to-be-discovered information categories in the DMN has
been argued to mediate an evolutionarily conserved function for the individual.
This is all the more likely becaus
the DMN has been shown to contain the two
...
to describe, quantify, and experimentally test
autonomously acting systems like the brain
can be the combination of control theory and reinforcement learning (RL).
It is long known that, the more the external world is predictable,
...
Our theory proposes that the implication of the DMN in a diversity of human cognitive processes
...
We show that the DMN is endowed with sufficient neurobiological equipment to
implement a reinforcement learning agent, at least in principle.




\section{Known Neurobiological Properties of the Default Mode Network}
We begin by a deconstruction of the overall DMN function
by reviewing published findings from neuroscience experiments
on the mechanistic relevance of each individual network node.
%
\subsection{The posteromedial cortex: Global monitoring and integration}
The posteromedial cortex (PMC) and
the dorsomedial prefrontal cortex (dmPFC) within the DMN
are probably responsible for the highest metabolic turn-over
...
sleep, and coma \citep{cavanna2006precuneus}.
%
...
to resemble "inner minds eye" \citep{cavanna2006precuneus}.
...
as well as by the inability to direct action towards
...
of various sensory inputs in rabbits and humans
\citep{vann2009does}.
Regarding perspective frames, this PMC subregion has been
proposed to mediate between the organism's egocentric viewpoint
(i.e., focused on sensory input) and
allocentric viewpoint (i.e., focused on world knowledge) 
in animals and humans
\citep{epstein2008parahippocampal, burgess2008spatial, valiquette2007different}.


The overall PMC may consequently monitor the subjective outcomes
...
In doing so, the PMC needs to continuously adapts to changes
...
The dmPFC subserves predominantly ambiguous processes
...
of one's own and other individuals' actions and thoughts.
Generally,
...
neural activity in the dmPFC
preferentially models stimulus-value associations of
possible actions that are not actually executed,
whereas neural activity
in the ventral portion of the medial PFC models value in the environment that leads performed actions
\citep{nicolle2012agent}.

The ventromedial prefrontal cortex (vmPFC) subserves
evaluation of subjective value as well as
reward and risk estimation of environmental stimuli.
In contrast to the dmPFC,
the vmPFC node of the DMN is more closely associated with
orchestrating adapted behavior by bottom-up-driven
processing of “what matters now”,
probably drawing on internal value representations
...
The vmPFC is preferentially connected with areas of the
limbic and reward systems.
...
was related to state encoding in this part of the brain.
...
brain region, to complex decision making
in humans and monkeys \citep{behrens2009computation}.
%
...
Such mechanisms of the hippocampus could also make important contributions to the
...
%
The HC thus reenacts elements of experienced environmental aspects for
consolidations and potentially, for integration into
...
\section{Reinforcement Learning, a computational model for DMN function}
We now argue that the DMN is equipped with sufficient neurobiological substrates gradient
for implementing all the major components (memory, reward signaling, accumulators, random sampling, etc.)
of an efficient \textit{reinforcement learning (RL)} algorithm, and all these
...
\textit{environment}. In a general RL framework, an \textit{agent}
interacts with an  environment in a trial-by-error fashion by taking
actions $a$, which trigger changes in the
...
rational probabilistic planning can reformulated
...
signal which is tried alongside each transition in the environments state-space, and maintains a probability distribution over the set of possible
...
In contrast, \textit{model-based RL} starts off with some mechanistic assumptions about the dynamics of world.
These can be assumptions about the physical laws governing the agent's world or constraints on the state-space and transitions thereupon.
For example, if a rat in a maze knows
...
\textbf{XXX last sentence not statistically sound}
\textbf{XXX this paper is mostly about the human DMN, therefore it would be better to rephrase the example into something with a human being}
We formalize
...
\subsection{Value mapping and action policies}
...
\begin{mdframed}
  ...
  the NAc is involved in reward evaluation.
\end{mdframed}
  The  goal of RL is then adapt this policy so as to  maximize the cumulative rewards. In
...
as potentially implemented in the DMN. For there to be any learning at all, it is thus necessary that $0 < \gamma < 1$.
% \footnote{In which case we can take $T \rightarrow +\infty$, provided the reward function $r$ is bounded.}.
Thus, the agent is positively biased towards considering rewards which with
...
expected cumulative value of state-stat-pairs $(A_0,S_0)$, also known as the \textit{Q-value}, namely
\begin{equation}
  \label{eq:qval}
  Q^{\pi}(s,a) = \mathbb E [G^\pi|S_0=s,A_0=a,A_1 \sim \pi,A_2 \sim \pi,\ldots].
\end{equation}
\begin{mdframed}
In plain English, the Q-value $Q^{\pi}(s,a)$ is the expected future reward
\textbf{XXX let's stick to "expected" in these situations because
it implies that the DMN is doing prediction without needing to 
emphasize it too much all the time}
over all possible
\textbf{XXX action???}
trajectories in which the agent starts off starts at with an environment in state
$s$, takes and action $a$, and then follows the policy $\pi$ thereafter to select future actions.
\end{mdframed}
...
\textbf{XXX coputationally or statistically?}
due to the enormous number of possible
\textbf{XXX action???}
...
% \end{mdframed}
One popular simplification that removes
%this issue
\textbf{excess in complexity on the agent's horizon???}
is to only consider the sub-family of
...
% table.
The \textit{Bellman's equation} \cite{sutton1998reinforcement} then takes the simple form
\begin{equation}
  \begin{split}
...
\begin{mdframed}
In the brain, such an experience replay mechanism is probably implemented in the hyppocampus (HC). Indeed,
hippocampal replay is a phenomenon observed in rats, mice, cats, rabbits, songbirds, and monkeys. Indeed, during sleep or awake rest, replay refers to the re-occurrence of a sequence of cell activations that also occurred during activity, but the replay has a much faster time scale.
\end{mdframed}
...
feature space. Such are the techniques that have been proposed by \cite{songNIPS2016}, for example.
%  and Parr et al. 2003 (JMLR)].
A breakthrough alternative proposed by Google DeepMind \cite{mnih2015,silver2016mastering}, is to learn this
...
\subsection{Hippocampal replay = inverse reinforcement-learning ?}
%\begin{mdframed}
...
pioneered in the 1950s by Richard Bellman and colleagues. To
...
or some approximation scheme like reinforcement learning.
...
%
From the computational viewed of a MDP,
