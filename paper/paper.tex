...
\textbf{XXX (danilo) this section needs more tight integration of the parts, and no just a separate independent subsection for each portion of the DMN...}
...
Reinforcement learning is a problem-solving technique in which, through their interactions with an external environment,
an agent learns to reach a goal, optimize a reward signal, or minimize some cost,
in an iterative trial-by-error fashion.
At any moment, each action $a$ taken triggers a changes in the
...
The environment is generally taken as stochastic, i.e unpredictable, changing in random ways. In addition,
...
Thus, the agent is more considering of rewards in to come in a short future, paying lesser attention to
rewards to come in at a later time.
...
\textit{deterministic policies}, which map each state to the best single best action to
...
\mapsto {Q}(s, a|\theta^Q_{k})$. Here, $Q(.,.|\theta^Q_{k})$ is an approximator for the $Q$-value operator,  parameterized by weights $\theta^Q_{k+1}$
(synaptic strengths, etc.).

Biologically, experience replay would be manifested as the re-occurrence of a sequence of cell activations that also occurred during activity.
%, but the replay has a much faster time scale.
In the human brain, such a mechanism is likely to be implemented by the
hyppocampus (HC), as experiments on various animals indicate this is the case.
Indeed, this phenomenon has been observed in rats, mice, cats, rabbits, songbirds, and
...
\begin{itemize}
  ...
  although this may not be neurobiologically plausible \textbf{XXX (danilo) what is not plausible. Be specific.}
  \item In sum,
  ...
  prediction coding hypothesis. \textbf{XXX (danilo) Reformulate. Also in general, you should be clear whether
    you're talking about MDPs, POMDPS, both, or neither. These are being mixed up a lot.}
\end{itemize}
...
\begin{itemize}
  ...
  \item While mental scene construction implies a distinction between
    environmental perception and internally generated mind-wandering\footnote{
      \textbf{XXX (danilo) This is more than just a footnote. Make this part of the paper
...
\textbf{XXX (danilo) Is this a loose opinion or a fact ? What do you mean by ``probably better models of the world'' ?}  Yet, the MDP framework provides an algorithmic explanation in that
...
\textbf{XXX (danilo) Your focus in this subsection ought to be on the SC itself not POMPDs, etc.}
 \begin{itemize}
    ...
    encountering a salient environmental stimulus. \textbf{XXX (danilo) remove the next sentence. It's probably false.}
    In contrast,
