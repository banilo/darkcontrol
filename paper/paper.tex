...
\subsection{Towards a computational account of DMN function}
...
This paper proposes a process model that satisfies this contention.
...
based on experimental findings in neuroscience. This walkthrough across individual network nodes will describe their mechanistic relevance, paving the way for their functional interpretation within our computational account in the next section.
...
$r = r(s, a)$ (or regret \textbf{XXX exchange by punishment?},
...
\begin{equation}
  y_k := r_k + \gamma \max_{a'} \tilde{Q}(s', a'|\theta_{k-1}),
\end{equation}
with the output of a parametrized regression model $(s,a)
\mapsto \tilde{Q}(s, a|\theta_{k-1})$. Here the function $(s,a) \mapsto Q(a,s|\theta_{k-1})$
...
\begin{equation}
  ...
  = -\mathbb E_{(s, a, r, s') \sim \mathcal U(D)}[\underbrace{(\tilde{Q}(s, a|\theta_{k}) - y_k)}_{\text{prediction error}}
...
the present formal account can parsimoneously incorporate
