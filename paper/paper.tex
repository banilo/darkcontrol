...
\author{Elvis Dohmatob, Guillaume Dumas, Danilo Bzdok\\
  \textbf{Please add your affiliation}
  %% INRIA, Parietal team, Saclay, France\\
  %% CEA, Neurospin, Gif-sur-Yvette, France\\
  %% firstname.lastname@inria.fr
  %% GD1 Institut Pasteur, Human Genetics and Cognitive Functions Unit, Paris, France
  %% GD2 CNRS UMR 3571 Genes, Synapses and Cognition, Institut Pasteur, Paris, France
  %% GD3 University Paris Diderot, Sorbonne Paris Cité, Paris, France
  %% GD4 Centre de Bioinformatique, Biostatistique et Biologie Intégrative (C3BI, USR 3756 Institut Pasteur et CNRS), Paris, France
}
...
Such functional roles become more likely due to
this brain network's highest energy consumption in the brain and
its intimate coupling with conscious awareness.
%
The present concept paper proposes a \textit{process model}
in trying to explain \textit{how} the DMN may actually implement continuous
...
previously proposed cognitive accounts as special cases and
parsimoniously explains
experimental findings in animals and humans.
%
A process models for the neural implemention
of continuous future predictions probably realized in the DMN
could offer insight into the statistical mechanisms underlying human behavior.
\end{abstract}
...
The baseline energy demand is subject to surprisingly little modulations
when mentally processing environmental challenges
\citep{raichle2001pnas}. \textbf{(XXX Guillaume: What about the energy consumption compare to sleep? Danilo: I'd love to cite that
but I am not aware of such studies)}
This dark matter of brain physiology begs the question
of underlying biological purpose
...
The DMN may exert higher-order control on human behavior
...
The human DMN accounts for a large part of the brain's baseline activity
and typically decreases in activity during psychological experiments.
...
%
Gaining insight into the overarching DMN function is challenging because
...
In this work,
we consider an agent faced with the choice of the next action
...
These ``offline'' processes may however contribute to optimized control of the organism. \textbf{(XXX Guillaume: What about the idea of model free control being also remote of environment? - danilo: hmm, let's keeping thinking about this)}
% We formalize
...
Even (necessarily imperfect) memories, random day-time mind-wandering, and dreams during sleep
...
outline their individual functional profiles,
paving the way for their functional interpretation within our computational account
...
are responsible for the highest turn-over
...
the inability to bind various individual features of the visual
...
repeatedly implicated in approach-avoidance behavior
\citep{vann2009does}.
...
promoted exploratory action towards actions
that were previously shunned \citep{hayden2008stim}.
...
dmPFC activity preferentially models stimulus-value associations of
possible actions that are not actually executed,
whereas vmPFC activity models value in the environment that leads performed actions
\citep{nicolle2012agent}.


Comparing to the dorsomedial prefrontal DMN,
the ventromedial portion of the prefrontal cortex (vmPFC) probably subserves
subjective-value-related evaluative processes and
risk estimation of self-relevant environmental stimuli.
...
% social competence and social network complexity, which are probably
...
but also imagining about one's own future and
 new fictitious experiences more broadly \citep{hassabis2007patients}.
...
distinct locations in space during environmental navigation.
...
But encoding and generative reconstruction in the HC extends
...
In this way, the HC may influence
...
%
Inner speech also hinges on knowledge retrieval
about the physical and inter-personal world.
Indeed,
the internal production of
verbalized thought ("language of the mind") was closely related to the left TPJ
...
and potential functions of the DMN
...
that are plausible to intertwine in the DMN.



RL is a problem-solving technique in which, through interactions with an external environment,
an agent learns to reach goals, optimize reward signals, and minimize costs
in an iterative trial-by-error fashion.
At a given moment, each taken \textit{action} $a$ triggers a change in the
...
can be parsimoniously explained as probabilistic simulations in form of mental scenes coupled with prediction error minimization
...
We propose that the brain in general and the DMN in particular
...
value estimations and action transition schedules drive
