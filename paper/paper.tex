...
top-down modulation realized by more multi-modal and associative brain regions
further away from the primary sensory cortices.
...
an internally generated prediction of aspects of sensory input is
...
why motor action is probably intimately related to sensory expectations
\citep{wolpert1995internal, kording2004bayesian}.
Contextual sensorimotor integration of perception-action cycles
may hence be maintained by top-down modulation
by a-priori information about the environment.



This process permits updates of the internal representation of the environment
to best accommodate and prepare for the constant influx of sensory examples.
There are hence a number of common properties
between the predictive coding account
and the proposed formal account.
Importantly,
the generative model of how perceived sensory cues arise in the
world would be incorporated into
the current neuronal wiring.
Further,
both frameworks are plausibilized by neurobiological evidence that suggest
the human brain to be a ``statistical organ'' with the biological purpose to
generalize from the past to new experiences.
Both accounts also provide a parsimonious explanation why the
human brain decreases processing load devoted to incoming information
when the environment is predictable and allocates increased
computation resources when novel stimuli or unexpected events are encountered.
The predictive coding and formal account
naturally evoke a mechanisms of brain plasticity in that
...
various hierarchical levels of sensory input processing. Instead,
the Markov property of our sequential decision making framework.
In our RL view, the environmental model is
...
explicitly parameterization of the agent's horizon and
...
and
more explicitly reconciles sensory input processing and action output preparation.
%
As such, recasting DMN function in terms of MDPs naturally incorporates
