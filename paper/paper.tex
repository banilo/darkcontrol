...
implement a reinforcement learning (RL) agent, at least in principle. A global
picture of the current body of knowledge (predictive coding, free-energy principle,
sentinel hypothesis, etc.) concerning decision-making in the brain
is drawn, with the DMN at its core. Our work ...
...
\begin{mdframed}
\textbf{Model-free RL in the brain is plausible.}  Indeed, as was noted in \cite{dayan2008decision}, It has long been suggested in the literature that there
  is a rather direct
mapping of model-free RL learning algorithms
onto the brain, with the neuromodulator dopamine serving
...
For example if a rat in a maze knows, for sure,
that staying still will produce no change in the environment, and in particular will not eventually lead to finding the food.
...
\subsection{Long-term rewards and policies}
...
\end{equation}
In plain English, the Q-value $Q^{\pi}(s,a)$ is the expected / average / mean future reward
over all possible trajectories in which the agent starts off starts at with an environment in state
...
provides a formalization of optimal selection.
...
\subsubsection{Q-learning} Q-learning (Watkins \& Dayan, 1992), a commonly used off-policy algorithm, uses the
\textit{greedy policy}
\begin{equation}
...
\subsubsection{Interlude: zoom on the Hippocampus}
...
Monte-Carlo simulation provides a simple method for evaluating a state $s$.
...
In the brain, the HC would generate / imagine sequences of world states, observations and rewards. These simulations would be used to update the value function, without ever looking inside the black box describing the model's dynamics. This would be a simple control algorithm by evaluating all legal actions and selecting the action with highest evaluation.
...
In our proposed RL-based view, \textit{inference} in the human brain reduces to generalization of
  ...
  Plasticity in the DMN also arises naturally:
...
In Friston's free-energy principle (FEP) \citep{friston2010free,fristonAIorRL}, the brain emerges as biomechanical inference engine which must minimize the long-term average of surprise.
...
On the surface, a common point between the FEP and our proposed RL-based framework
  ...
  rewarding outcomes in the future. A ``free-energy'' agent is barely a biomechanical machine which has the tendency to resist internal phase-transitions which lead contradictory with the
  blurred version of the external world captured as sensations
