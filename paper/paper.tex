...
interacts with an unknown environment in trial-and-error fashion by taking
\textit{actions / decisions} $a_t$ which trigger changes in the
\textit{state} of the environment
$s_t \rightarrow s_{t+1}$, accompanied with a real-valued \textit{reward}
$r_t$ (or regret!, since it can be negative). This reward should be thought
...
One popular simplification that removes this issue is to only consider the sub-family of \textit{deterministic policies}, which map each state to the best single best action which should be taken at that state, and therefore define functions from states to actions.
...
\textit{Bellman's equation} [Sutton and Barto 1998]
\begin{equation}
