...
\author{Elvis Dohmatob\textsuperscript{1,2}, Guillaume Dumas\textsuperscript{3,4,5,6}, Danilo Bzdok\textsuperscript{1,2}
}
...
\small{
    ...
    \textbf{3} Institut Pasteur, Human Genetics and Cognitive Functions Unit, Paris, France\\
    \textbf{4} CNRS UMR 3571 Genes, Synapses and Cognition, Institut Pasteur, Paris, France\\
    \textbf{5} University Paris Diderot, Sorbonne Paris Cité, Paris, France\\
    \textbf{6} Centre de Bioinformatique, Biostatistique et Biologie Intégrative, Paris, France
}
...
its intimate coupling with conscious awareness are both pointing towards an important functional roles.
%
Many research streams support its evolutionarily adaptive role as
envisioning mental scenarios to simulate the future.

The present concept paper proposes to explain \textit{how} the DMN may actually implement such continuous assessment and prediction of the environment to guide action choices.
...
previously proposed cognitive accounts such as \textit{predictive coding}.
%
A process model for the neural implemention
of continuous future predictions potentially realized in the DMN
could offer insight into the statistical mechanisms underlying human behavior and provide a parsimonious explanation of experimental findings in animals and humans.
\end{abstract}
...
decoupling of brain structures discarded the idea of the DMN being only a passive network resonance and rather supported its important role in the sustenance of conscious awareness \citep{horovitz2009decoupling}.
This dark matter of brain physiology strongly question
its underlying biological purpose
...
This set of some of the highest regions in
...
The human DMN is responsible for a large part of the brain's baseline activity,
...
In line with this, the DMN contains hotspots
of highest energy consumption in the entire central nervous system
\citep{raichle2001pnas},
which persists to a substantial degree during
...
a human agent faced with the choice of the next action
and being guided by rich
reenactment of really-happened, hypothetically conceived, and
estimated futures to optmize behavioral performance.
...
Without requiring explicitly awareness, those ``offline'' processes may however contribute to optimized control of the organism. \textbf{(XXX Guillaume: What about the idea of model free control being also remote of environment? - danilo: hmm, let's keeping thinking about this - Guillaume: where shall we add this, more in the discussion, then?)}
% We formalize
...
\subsection{Towards a formal account of DMN function}
...
Model-free RL is plausibly realizable in the human brain. Indeed,
...
\begin{mdframed}
\begin{definition*}
% \textbf{XXX Danilo: Is there a definition 2 somewhere?}
...
\subsection{Expected values, Bellman equation, and optimal control}
...
% \textit{greedy policy}
Optimal behavior corresponds to a strategy $\pi^*$ for choosing actions such that, for every state, the chosen action guarantees the best possible reward on average. This can be formalized by
...
\end{equation}
the optimal Q-value over all policies. The goal of learning is to approach the policy $\pi^*$ as ``close as possible'', that is to \text{solve} the POMDP. Of course \eqref{eq:qlearning} is just a definition and
...
% table.
Fortunately, the \textit{Bellman equation} \citep{sutton1998reinforcement} provides a fixed-point relation which defines $Q^*$ implicitly via a sampling procedure, without the intractable problem
over all every policy, viz
\begin{equation}
  Q^* = \text{Bel}(Q^*),
  \label{eq:bellman}
\end{equation}
...
\begin{equation}
  \begin{split}
   ...
   &= r(s,a) + \mathbb E_{s' \sim p(s'|s,a)} [\gamma \max_{a' \in \mathcal A}Q(s', a')]\\
...
% The optimal Q-value operator $Q^*$
% is a fixed point for this equation.
...
\begin{mdframed}
  \textbf{ NCs for the Bellman equation:}
  ...
\end{mdframed}

\subsubsection{Value approximation and greedy action consideration, and experience replay}
...
\begin{equation}
  ...
  \label{eq:policy}
\end{equation}
by querying an approximate version $a \mapsto \tilde{Q}(a,s|\theta)$ of the $Q$-value function ``frozen'' at state $s$. In the DMN, The dmPFC (BA9) would implement such a greedy lookup over the action space.
The tensor $\theta$ encodes the model
  parameters and would correspond to synaptic weights and / or connection strengths between
  DMN regions. It is a time-varying neuronal program which dictates how to move from world states $s$ to actions $a$ via the greedy policy \eqref{eq:policy}. Learning consists in rewriting this program on the fly, so that the approximating functional $\tilde{Q}(.,.|\theta)$ becomes closer and closer to the optimum $Q^*$.

The action $a$ taken in \eqref{eq:policy} triggers a reward $r$ collected by the agent,
...
and a loss function is in demand. The Bellman equation \eqref{eq:bellman}
provides a way to produce such a loss function \eqref{eq:oracle} as we now outline below.
...
\subsubsection{Stochastic gradient descent in the DMN}
...
Over time (i.e in the limit $k \rightarrow \infty$), the approximate
$Q$-values $\tilde{Q}(a,s|\theta_{k-1})$ provably\footnote{The proof is standard and relies on the Banach Fixed-Point Theorem applied to the Bellman transform.}
converge uniformly to the unique fixed-point $Q^*(a,s)$ defined implicitly by the Bellman
...
from stored prior information.
At each stage of neural processing,
an internally generated prediction of aspects of sensory sensation is
...
Both frameworks model the consequences of action.
The predictive coding framework is typically explained without
...
in order to constantly improve action in our dynamic world.
