...
\begin{equation}
  ...
  \label{eq:bellman}
\end{equation}
This \textit{fixed-point} equation provides a recursive decomposition of optimal behavior.
...
and the environment transitions to a new state $s' \in \mathcal S$.
...
would correspond to the strengths of low-level synaptic or high-level functional connections between different elements
in brain networks.
\textbf{XXX perhaps add one sentence to rephrase and detail the idea}

Learning (i.e., fitting)
of the the entire model parameters can effictively be achieved via \textit{backprop},
...
observations and rewards. These simulations would be used to update the value function, without ever looking inside the black box describing the model's dynamics. This would be a simple control algorithm by evaluating all legal actions and selecting the action with
...
\paragraph{Hippocampal replay corresponds to inverse reinforcement-learning?}
%\begin{mdframed}
...
in order to constantly improve present action in the world.
