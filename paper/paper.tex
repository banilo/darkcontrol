...
In the present manuscript, we propose a \textit{process model}
that attempts to explain \textit{how} the DMN may implement
continuous evaluation and prediction of the environment to guide behavior.
%
We first review the rich literature on the DMN and recognize that the DMN contains the sufficient neuro-biological substrates to implement a full-blown reinforcement-learning agent, at least in principle. DMN function is then recast in mathematical terms of
...
to allow for the discovery of a unique brain network that probably subserves
...
The human DMN is responsible for extended parts of the baseline neural activity,
...
entertain probabilistic contemplations
...
This idea is invigorated by empirical evidence from
...
\citep{Averbeck2017,amygdala_bzdok}.
...
Consequently, the PMC may contribute to overall DMN function by
``altering behavior in response to unexpected change''
\citep{pearson2011posterior}.
This DMN node probably
monitors sudden and gradual environmental changes in subjective outcomes
of action choices for integration
with memory and perspective frames
to improve short- and longer-term behavioral agendas.
...
an agent learns to reach goals and optimize reward signals occasionally emitted by the environment, in an iterative trial-and-error fashion (Fig. \ref{fig:rl}).
...
a particular action.  Thus the agent learns behaviors.
...
state are accessible to the agent's sensory perception~\citep{starkweather2017dopamine,o2006making,o2006making}.
...
\subsection{Interlude: Rethinking the Action-State-Reward model of RL}
Barto proposes to decouple extrinsic rewards (i.e generated by the external environment) from intrinsic reward (generated from withing the "organism")
biologically, extrinsic rewards would correspond to so-called "sensations"
while intrinsic rewards would correspond to "reward signals".

Thus there are two environments: the external environment, and the internal environment built-into the "organism". The organism takes highly level DECISIONS (the DMN can confortably sit here!) which act on the internal environment (this environment probably corresponds to low-level subsystems like motor, etc.) which then transform theses  into ACTIONS executed in the external environment. The external environment generates sensations which get mapped into reward signals (via the internal environment), etc.

The interesting thing is that with this rethinking, the organism can have a perception-action loop even in the absence of external stimulus.
...
\paragraph*{Answer:} We emphasize that the brain in general, and the DMN in particular,
...
This adds the capacity for memory, since then, the next state depends not only on the current state but also on a finite number of past states. This is precisely what partially observable MDPs do ~\citep{starkweather2017dopamine,o2006making,o2006making}.

Also, it should be noted that MDPs have been successfully used as neural model for animal behavior. One can mention ~\citep{Rao2010}, just to name a few.
\end{mdframed}
...
As a consequence of this decomposition, the complicated problem \eqref{eq:qlearning}
is broken down into simpler sub-problems at different time points.
Indeed, exploitation of hierarchical structure in action considerations
...
representation using a deep neural-network model, leading to the
...
Stochastic here means that the true gradient is estimated from batches of training examples samples,
...
highest expected cumulative rewards.As a result, quick-to-learn instance-based control policies serve as a rough approximation while a slower more generalisable decision-making system system is trained up~\citep{pritzel2017neural,lengyel2008hippocampal}.
...
\begin{figure}[!h]
  ...
  Overview of how the constituent nodes of the DMN may
...
may enable learning about world aspects by reconciling
...
% INTRO
According to theories of the free-energy principle (FEP) and
active inference \citep{friston2010free, fristonAIorRL},
...
The FEP also proposes a companion model called the \textit{recognition} model,
which works in tandem with the generative model $p_R(\z|\x)$ to accomplish approximate inference. Put differently, the recognition model dreams imaginary worlds $\z$,
while the generative model tries to produce sensory sensations $\x$ which match these
imagined mental scenarios.
Howewer, surprise is challenging to optimize numerically
because we need to sum over all hidden causes of the sensations.
Instead, FEP therefore minimizes an upper-bound of surprise,
namely the \text{free energy} $F_G^R(\x) := \langle \log p_G(., \x) \rangle_{p_R(. |\x)} - \langle \log p_R(. |\x))\rangle_{p_R(. |\x)}$ given by
...
some of which outlined in the following.
...
a crucial drawback of the wake-sleep algorithm is that it involves a pair
of forward (generation) and backward (recognition) models that
...
by using a reparametrization trick. For instance, unlike the wake-sleep algorithm
for minimizing free energy, VAEs can be efficiently trained via back-propagation of prediction errors. Most importantly, the reparametrization trick allows for a drastic reduction in the variance of the in estimates of the model's gradients, leading to effective learning, something which was not possible in wake-sleep\citep{dayan1995helmholtz}. The details of this trick can be found in \citep{kingma2013auto}.
...
Finally, we note that FEP and active inference can be reframed in terms of
...
the agent has about the world. More explicitly, this corresponds to

\begin{eqnarray*}
  -Q^{\text{FEP}} \sim \underbrace{\log (p_G)}_{\text{FEP generative surprise}}.
    \end{eqnarray*}
...
under the equivalent framework of information compression \citep{schmidhuber2010,mohamed2015}.
...
%
This functional interpretation may be well compatible with the DMN's
