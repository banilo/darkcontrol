...
\begin{eqnarray}
  \begin{split}
    ...
    &= \langle E_G(., \zeta) \rangle_{p_G(. |\zeta)} - \mathcal H(p_G(. |\zeta))
  \end{split}
  \label{eq:helm}
\end{eqnarray}
...
so that one recognizes The last quantity in \eqref{eq:helm} as the the \textit{Helmholtz free-energy} (at unit temperature!), defined formally by
\begin{equation}
  F_G(\zeta) := \langle E_G(., \zeta) \rangle_{p_G(. |\zeta)} - \mathcal H(p_G(. |\zeta)).
\end{equation}
%% , modelled as the physical system for which $p_G(\psi|\zeta)$ dictates the occupation probabilities of macrostates $\psi$
...
Whence, for any macrostate $\psi$, we have $p_G(\zeta) = Z_G(\zeta) = \exp(-E_G(\psi, \zeta)) / p_G(\psi|\zeta)$, and so it holds that
\begin{equation}
F_G(\zeta)\overset{\eqref{eq:helm}}{=} -\log(p_G(\zeta)) = -\log(Z_G(\zeta)) = E_G(\psi, \zeta) + \log(p_G(\psi|\zeta)).
\end{equation}
Now, in the above equation, the LHS only depends on the generative model $G$ and the data point $\zeta$:
it doesn't depend on the hidden variable $\psi$, etc. So, taking expectations w.r.t an arbitrary
density\footnote{The conditioning in $P_R(.|\zeta)$ is because this density is selected from a world in which the sensory inputs and internal brain
  state vector $\zeta$ is assumed already observed.} $p_R(.|\zeta)$ yields
\begin{equation}
  \begin{split}
