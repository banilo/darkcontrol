...
\textit{Q-learning} (Watkins \& Dayan, 1992), a commonly used off-policy \textit{critic-only} algorithm, uses the
\textit{greedy policy}
\begin{equation}
...
\subsection{Q-learning with value approximation and replay}
...
At state $s \in \mathcal S$, the agent takes an action $a \in \mathcal A$
...
this cycle, a new \textit{experience} $e = (s,a,r,s')$ is produced; this represents an exemplar behavior of the agent and is recorded in replay memory:
